Shader "Hidden/Dithering"
{
	Properties
	{
		...
		_ColorDepth("Color Depth", Float) = 4
		...
		SubShader
		{
			...
			Pass
			{
				...
				fixed4 frag(v2f i) : SV_TARGET
				{
					...
					pixelCoord = floor(pixelCoord / _PixelSize);
					float2 cord = pixelCoord * _PixelSize / _ScreenParams.xy;
					...
					float newBrightness = brightness + dither * _DitherStrength * 0.01;
					fixed4 color = col / brightness * round(newBrightness * _ColorDepth) / _ColorDepth;

					col += dither * _DitherStrength * 0.01;
